# ہفتہ 3: Python کے ساتھ ROS 2 پیکجز بنانا
## کا تعارف
اس ہفتے، ہم سیکھیں گے کہ Python کا استعمال کرتے ہوئے پیشہ ورانہ ROS 2 پیکجز کو کس طرح ترتیب دینا، بنانا اور تعینات کرنا ہے۔ آپ ایک مکمل پیکج تخلیق کریں گے جس میں پبلشرز، سبسکرائبرز، سروسز، اور لانچ فائلیں شامل ہوں گی جو متعدد نوڈز کو منظم کرتی ہیں۔
...
یہ پیدا کرتا ہے:
- `package.xml` - انحصار اور میٹا ڈیٹا
- `setup.py` - نوڈز کے لیے انٹری پوائنٹس
- `setup.cfg` - پائتھن پیکیج کی تشکیل
- `humanoid_control/` - پائتھن ماڈیول ڈائریکٹری
## پیکیج کی معلومات: package.xml
```xml
<?xml version="1.0"?>
...
**اہم سیکشنز**:
- `data_files`: غیر Python فائلیں انسٹال کریں (launch, config, URDF)
- `entry_points`: Python اسکرپٹس سے بنائی گئی ایگزیکیوٹ ایبلز
## مثال نوڈ: جوائنٹ اسٹیٹ پبلشر
`humanoid_control/joint_controller.py` بنائیں:
```python
#!/usr/bin/env python3
import rclpy
...
## بہترین طریقے
### پیکیج کی تنظیم
- ہر روبوٹ یا عملیاتی یونٹ کے لیے ایک پیکیج
- علیحدہ انٹرفیس پیکیجز (پیغامات/سروسز)
- متعلقہ پیکیجز کو گروپ کرنے کے لیے میٹا پیکیجز کا استعمال کریں
### ورژن کنٹرول```bash
# .gitignore
build/
install/
...
- pytest کے ساتھ ٹیسٹنگ
- ڈیبگنگ کی تکنیکیں
## آگے کیا ہے؟
**ہفتہ 4: ہیومینائیڈ روبوٹ کی تفصیلات کے لیے URDF** - URDF اور Xacro کا استعمال کرتے ہوئے روبوٹ کی جیومیٹری اور کائنیماتکس کی وضاحت کرنا سیکھیں۔
